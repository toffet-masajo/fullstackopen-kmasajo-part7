...
describe('adding new blog entry', () => {
  ...
  test('title attribute missing', async () => {
    ...
    await api.post('/api/blogs').set('Authorization', `Bearer ${response.body.token}`).send(newBlog).expect(400);
  });
...
describe('deleting blog entry', () => {
  test('delete existing blog entry', async () => {
    ...
    await api.delete(`/api/blogs/${blogToDelete.id}`).set('Authorization', `Bearer ${response.body.token}`).expect(204);
...
describe('updating blog entry', () => {
  test('update existing blog entry', async () => {
    ...
    await api.put(`/api/blogs/${oldBlogs[0].id}`).send(blogToUpdate).expect(200);
  ...
  test('update non-existing blog entry', async () => {
    ...
    await api.put(`/api/blogs/${id}`).send(blogToUpdate).expect(400);
  });
});
